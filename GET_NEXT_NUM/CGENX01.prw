...
Exemplo:
Colocar a função abaixo no inicializador do campo de codigo a ser utilizado
...
@param _cInc           , Caractere, Paramentro de inclusão, caso informado irá considerar apenas os iniciados com esse caracteres
		baseado no campo passado no segundo parametro, pode ter mais de uma caractere
@param _cExc           , Caractere, Parametro de exclusão, caso informado NÃO irá considerar os codigos iniciados com os caracteres passados
		baseado no campo passado no segundo parametro, pode ter mais de uma caractere
@param _lCompartilhado , Lógico, Caso true força a busca do codigo de forma compartilhado, mesmo para tabelas exclusivas.
...
User Function CGENX01(_cTab, _cCampo, _cInc, _cExc, _lCompartilhado)
	...
	If Type("HndCGENX01") <> "N"
		// Variavel publica, dessa forma caso apenas um usuario esteja acessando o arquivo vai poder ser deletado e com isso a numeração fica correta
		// se mais de um usuario acessar ai não será possivel deletar o arquivo e com isso o proximo virá do arquivo não do select max
		Public HndCGENX01 := 0
	EndIf
	...
	Else
		If File(_cDir + _cFile)
			...
			If fErase(_cDir + _cFile) < 0
				ConOut(ProcName() + " - " + _cDir + _cFile + " Arquivo aberto por outra instancia, não será deletado dessa vez")
			Else
	...
	// abro em modo leitura e gravação porém não de forma exclusiva
	...
	// não fecho o arquivo pois dessa forma garanto que quando tiver mais de uma pessoa utilizando a rotina consegue pegar o proximo correto
